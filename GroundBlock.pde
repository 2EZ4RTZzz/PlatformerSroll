class GroundBlock extends Block{
  ...
  GroundBlock(PVector pos, int amount, PImage[] images, boolean hasBase) {
    super(pos, amount, images);
    
    if (hasBase)
      gb = new GroundBase(new PVector(pos.x - dim.x / 2 + imgs[0].width, pos.y + imgs[0].height / 2), tileAmount - 2, 4, groundBaseImg);
